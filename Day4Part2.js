var passphrases = `oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
var row = passphrases.split("\n");

var acceptablePhrases = 0;
...
console.log("There are: " + acceptablePhrases + " acceptable phrases");

function testPhrase(data) {
    ...
    for (var i = 0; i <= data.length; i++) {
        for (var j = 0; j <= data.length; j++) {
            ...
            // Check if any 2 words are the same
            if (data[i] === data[j]) {
                count++;
            }
        }
