...
var puzzle = `116	1259	1045	679	1334	157	277	1217	218	641	1089	136	247	1195	239	834
...
{   
    ...
    var theMax = getMax(col);
    var theMin = getMin(col);
    var theDiff = theMax - theMin;

    differences.push(theDiff);

    var theAnswer = 0;
    
    differences.forEach(function(answer){
        theAnswer += answer    
    });
    
    console.log("Final answer: " + theAnswer);
}

function getMax(arr) {
    var ret = Math.max( ...arr );

    return ret;
}

function getMin(arr) {
    var ret = Math.min( ...arr );
