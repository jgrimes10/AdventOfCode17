...
class Disc {
    constructor(name, weight, parent, children) {
        this.name = name;
        this.weight = weight;
        this.parent = parent;
...
parseDisc();
findParent(Disks[0]);
console.log(Disks[0].parent);

function parseDisc() {
    var inputArray = input.split("\n");
    //console.log(inputArray[0]);
    var disk = inputArray[0].split(" ");

    var name = disk[0];

    var weight = disk[1].replace('(','').replace(')','');
    ...
    for (var i = 3; i < disk.length; i++) {
        var child = disk[i].replace(',','');
        children.push(child);
    }

    var thisDisc = new Disc(name,weight,null,children);
...
function findParent(val) {
    for (var i = 0; i < Disks.length; i++){
        for (var j = 0; j < Disks[i].children.length; j++){
            if (val.name == Disks[i].children[j]) {
                Disks[Disks.indexOf(val)].parent = Disks[i].children[j];
            }
        }
    }
}